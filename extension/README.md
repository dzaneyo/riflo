...
```text
http://127.0.0.1:8787/#source=<编码后的完整播放列表地址>&referer=<编码后的当前页面>&from=extension
```

扩展不直接调用 riflo API，也不会自动开始下载。Web UI 仍由用户确认任务参数后提交。

候选请求会同时记录一个脱敏后可显示的来源。提取顺序是 `documentUrl`、`initiator`、`origin`（Firefox 的 `originUrl` 也会作为 `origin` 处理）；只接受 `http`/`https`，并移除来源中的 URL 用户信息和 fragment。来源的完整 URL 只在 `storage.session` 中保留，弹窗只显示去掉 query、fragment 和用户信息后的地址。点击「在 riflo 打开」时，优先使用候选请求来源作为 Referer；如果请求没有可用来源，才回退到当前顶层标签页 URL。候选的多次网络事件合并时，较精确的 `documentUrl` 不会被空值或较弱来源覆盖。

## 权限说明

- `webRequest`：只读监听请求和响应头，发现播放列表地址；请求阶段只接受明确的 `.m3u8` URL，响应阶段还会识别 HLS Content-Type。
...
- 弹窗展示播放列表 URL 和请求来源都会移除 query、fragment 和 URL 用户信息；「在 riflo 打开」仅在用户点击后使用 storage.session 中保存的完整播放列表 URL，并优先使用候选请求来源作为 Referer。
...
- 扩展只把当前页面 URL 作为 Referer 参数传给 riflo，不携带 Cookie。riflo 服务必须先在本机回环地址运行。
