...
- a Chrome and Firefox extension that discovers HLS candidates.
...
Use `--referer`, `--user-agent`, or `--cookie` when the media server requires normal request context. These values are used only for the current request and are not stored.
...
Play a video, open the extension, and send a detected HLS candidate to riflo. The extension does not read cookies or start downloads automatically.
